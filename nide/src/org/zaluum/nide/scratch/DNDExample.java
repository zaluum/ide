...
public class DNDExample {
	...
	private Control createWidget(int type, Composite parent, String prefix) {
		...
		case LIST: {
			...
			list.setItems(new String[] { prefix + " Item a",
					prefix + " Item b", prefix + " Item c", prefix + " Item d" });
			return list;
		}
		default:
			throw new SWTError(SWT.ERROR_NOT_IMPLEMENTED);
	...
	public void open(Display display) {
		...
		shell.setSize(Math.min(size.x, monitorArea.width - 20),
				Math.min(size.y, monitorArea.height - 20));
		shell.open();
