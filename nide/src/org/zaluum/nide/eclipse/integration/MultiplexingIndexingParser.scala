...
class MultiplexingIndexingParser
  (requestor : ISourceElementRequestor , 
      ...
      useSourceJavadocParser : Boolean ) extends IndexingParser(requestor, problemFactory, options, reportLocalDeclarations, optimizeStringLiterals, useSourceJavadocParser) {
	/*SourceElementNotifier notifier;
	boolean groovyReportReferenceInfo;
	ISourceElementRequestor requestor;
  */
  val mynotifier =  ReflectionUtils.getPrivateField(classOf[SourceElementParser], "notifier", this).asInstanceOf[SourceElementNotifier];
		//this.groovyReportReferenceInfo = reportLocalDeclarations;
		//this.requestor = requestor;

	override def setRequestor(requestor : ISourceElementRequestor ) {
	...
	override def parseCompilationUnit(unit :ICompilationUnit ,fullParse: Boolean , pm : IProgressMonitor ) : CompilationUnitDeclaration = {
		if (ContentTypeUtils.isZaluumLikeFileName(unit.getFileName())) {
			...
			visitor.doVisit(cud.currentPackage);
