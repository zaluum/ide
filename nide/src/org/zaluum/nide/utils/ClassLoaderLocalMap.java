...
public class ClassLoaderLocalMap implements Opcodes {
	...
	private static final Map<Object, Object> globalMap = Collections
	...
	private static boolean hasHolder(ClassLoader cl) {
		...
		try {
			Class<?> clazz = (Class<?>) findLoadedClass.invoke(cl,
	...
	private static Map<Object, Object> getLocalMap(ClassLoader cl) {
		...
		Class<?> holderClass;
		try {
			holderClass = (Class<?>) findLoadedClass.invoke(cl,
					new Object[] { holderClassName });
		} catch (IllegalArgumentException e) {
			throw new RuntimeException(e);
		} catch (IllegalAccessException e) {
			throw new RuntimeException(e);
		...
		if (holderClass == null) {
			...
			try {
				holderClass = (Class<?>) defineMethod.invoke(cl, new Object[] {
						holderClassName, classBytes, Integer.valueOf(0),
		...
		try {
			return (Map<Object, Object>) holderClass.getDeclaredField(
					"localMap").get(null);
		} catch (Throwable e1) {
			throw new RuntimeException(e1);
		}
	...
	private static byte[] buildHolderByteCode(String holderClassName) {
		...
		FieldVisitor fv;
		MethodVisitor mv;
		cw.visit(V1_2, ACC_PUBLIC + ACC_SUPER, holderClassName, null,
				"java/lang/Object", null);
		{
			fv = cw.visitField(ACC_PUBLIC + ACC_FINAL + ACC_STATIC, "localMap",
		...
		}
		{
			...
			mv.visitInsn(DUP);
			mv.visitMethodInsn(INVOKESPECIAL, "java/util/WeakHashMap",
					"<init>", "()V");
			mv.visitFieldInsn(PUTSTATIC, holderClassName, "localMap",
					"Ljava/util/Map;");
			mv.visitInsn(RETURN);
			mv.visitMaxs(2, 0);
			mv.visitEnd();
