...
object WidgetTemplateDump {
  ...
  def attachInitCode(mv: MethodVisitor, c: Compiled) {
    ...
    mv.visitFieldInsn(PUTFIELD, c.bcd.className.internal, "widget", "Ljavax/swing/JComponent;");
    mv.visitVarInsn(ALOAD, 0);
    mv.visitFieldInsn(GETFIELD, c.bcd.className.internal, "widget", "Ljavax/swing/JComponent;");
    ...
    for (box ← c.boxesInOrder; val typ = c.boxType(box); if (typ.visual)) {
      ...
      mv.visitFieldInsn(GETFIELD, box.boxClassName.internal, "widget", "Ljavax/swing/JComponent;");
