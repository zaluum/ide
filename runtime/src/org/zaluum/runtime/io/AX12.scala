...
class AX12_IO{
  ...
  /*def write_packet(packet : Buffer[Int]){
    val command = packet(0)
    val values = Buffer[Int]()
    val (register,parse2) = command match {
    	case AX_GOAL_POSITION => (AX_GOAL_POSITION_L,true)
    	case AX_GOAL_SPEED => (AX_GOAL_SPEED_L,true)
    	case AX_TORQUE_EN => (AX_TORQUE_ENABLE,false)
    }
    ...
    ser.sync_write_to_servos(register, values)
  }*/
  ...
  */
  ...
  def get_servo_feedback(servoId : Int) = {
      ...
      err(response, " when getting feedback from servo with id " + servoId)
          ...
          (servoId, position, speed, sload, voltage, temperature, moving)          
  }
