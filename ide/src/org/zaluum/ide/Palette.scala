		...
		palette.add({
			def newTool(name:String, desc:String, resourceKey:String, modelClass:Class[_], baseResources:Class[_]):CreationToolEntry = {
				...
				new CreationToolEntry(name, desc, new CreationFactory() {
					...
					}, null, null)//ir.getDescriptor(resourceKey + "_16"), ir.getDescriptor(resourceKey + "_32"));
			}
			...
			//boxToolbar.add(newTool("Composed Box","A Box Composed of other boxes", "composed", ComposedVBox.getClass,Icons.getClass));
